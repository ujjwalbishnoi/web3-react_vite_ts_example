...
import { SupportedChainId } from "../params/chains";
import { CHAIN_INFO } from "../params/chainInfo";
...
export default async function switchToNetwork({
  ...
  try {
    ...
    if (
      ...
      try {
        console.log("switching after adding network................");
        await provider.request({
          method: "wallet_switchEthereumChain",
          params: [{ chainId: info.chainIdInHex }],
        });
      } catch (newError) {
        console.debug("Added network but could not switch chains", newError);
      }
    } else {
      throw error;
    }
